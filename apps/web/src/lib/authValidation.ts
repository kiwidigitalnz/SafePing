...
export interface AuthValidationResult {
  isValid: boolean
  status: 'authenticated' | 'pending_verification' | 'not_found' | 'error'
  redirectTo?: string
  message?: string
  metadata?: any
}
 ...
 */
export async function validateAuthState(email?: string): Promise<AuthValidationResult> {
  try {
 ...
 */
export async function checkVerificationStatus(email: string): Promise<AuthValidationResult> {
  try {
 ...
 */
export async function cleanupExpiredVerifications(email: string): Promise<void> {
  try {
