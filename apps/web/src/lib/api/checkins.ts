...
export async function updateCheckInStatus(id: string, status: 'safe' | 'overdue' | 'missed' | 'emergency') {
  const { data, error } = await supabase
    .from('check_ins')
    .update({ status })
    .eq('id', id)
...
export async function acknowledgeCheckIn(id: string, acknowledgedBy: string, notes?: string) {
  const { data, error } = await supabase
    .from('check_ins')
    .update({ 
      metadata: { 
...
async function getCurrentUserOrgId(): Promise<string> {
  ...
  if (error) throw error
  if (!userData?.organization_id) throw new Error('User not associated with organization')
