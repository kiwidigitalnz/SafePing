...
export const subscriptionService = {
  ...
  async createCheckoutSession(
    planId: string,
    successUrl: string = `${window.location.origin}/settings?tab=billing&success=true`,
    cancelUrl: string = `${window.location.origin}/settings?tab=billing&canceled=true`
  ...
  async createPortalSession(
    returnUrl: string = `${window.location.origin}/settings?tab=billing`
  ): Promise<{ url: string }> {
    const { data, error } = await supabase.functions.invoke('manage-subscription', {
      body: {
  ...
  async canAddStaff(): Promise<boolean> {
    try {
...
export const formatPrice = (cents: number, currency: string = 'NZD'): string => {
