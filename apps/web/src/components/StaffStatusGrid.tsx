...
function StaffStatusCard({ checkIn, onClick }: StaffStatusCardProps) {
...
interface StaffStatusGridProps {
  onStaffClick?: (checkIn: any) => void
}

export function StaffStatusGrid({ onStaffClick }: StaffStatusGridProps) {
  ...
  const handleStaffClick = (checkIn: any) => {
  ...
  if (latestCheckIns.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">No staff yet</h3>
        <p className="mt-1 text-sm text-gray-500">
          Invite your first staff member to start monitoring their safety.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-900">
          Staff Status ({latestCheckIns.length} staff)
        </h3>
      ...
      {/* Emergency Staff - Always show first */}
      {groupedStaff.emergency.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-red-600 mb-3 flex items-center">
            <AlertTriangle className="w-4 h-4 mr-1" />
            EMERGENCY ({groupedStaff.emergency.length})
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {groupedStaff.emergency.map((checkIn) => (
              <StaffStatusCard
                key={checkIn.id}
                checkIn={checkIn}
                onClick={() => handleStaffClick(checkIn)}
              />
            ))}
          </div>
        </div>
      ...
      {/* Missed Staff */}
      {groupedStaff.missed.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-red-600 mb-3">
            MISSED CHECK-INS ({groupedStaff.missed.length})
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {groupedStaff.missed.map((checkIn) => (
              <StaffStatusCard
                key={checkIn.id}
                checkIn={checkIn}
                onClick={() => handleStaffClick(checkIn)}
      ...
      {/* Overdue Staff */}
      {groupedStaff.overdue.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-yellow-600 mb-3">
            OVERDUE ({groupedStaff.overdue.length})
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {groupedStaff.overdue.map((checkIn) => (
              <StaffStatusCard
                key={checkIn.id}
                checkIn={checkIn}
                onClick={() => handleStaffClick(checkIn)}
      ...
      {/* Safe Staff */}
      {groupedStaff.safe.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-green-600 mb-3">
            SAFE ({groupedStaff.safe.length})
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {groupedStaff.safe.map((checkIn) => (
              <StaffStatusCard
                key={checkIn.id}
                checkIn={checkIn}
                onClick={() => handleStaffClick(checkIn)}
