...
function WorkerStatusCard({ checkIn, onClick }) {
...
export function WorkerStatusGrid({ onWorkerClick }) {
    ...
    const handleWorkerClick = (checkIn) => {
    ...
    if (latestCheckIns.length === 0) {
        return (_jsxs("div", { className: "text-center py-12", children: [_jsx(User, { className: "mx-auto h-12 w-12 text-gray-400" }), _jsx("h3", { className: "mt-2 text-sm font-medium text-gray-900", children: "No workers yet" }), _jsx("p", { className: "mt-1 text-sm text-gray-500", children: "Invite your first worker to start monitoring their safety." })] }));
    }
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("h3", { className: "text-lg font-medium text-gray-900", children: ["Worker Status (", latestCheckIns.length, " workers)"] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("div", { className: `w-2 h-2 rounded-full ${isConnected ? 'bg-green-400' : 'bg-gray-400'}` }), _jsx("span", { className: "text-xs text-gray-500", children: isConnected ? 'Live' : 'Offline' }), loading && (_jsx("div", { className: "animate-spin w-4 h-4 border-2 border-gray-300 border-t-primary rounded-full" }))] })] }), groupedWorkers.emergency.length > 0 && (_jsxs("div", { children: [_jsxs("h4", { className: "text-sm font-medium text-red-600 mb-3 flex items-center", children: [_jsx(AlertTriangle, { className: "w-4 h-4 mr-1" }), "EMERGENCY (", groupedWorkers.emergency.length, ")"] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4", children: groupedWorkers.emergency.map((checkIn) => (_jsx(WorkerStatusCard, { checkIn: checkIn, onClick: () => handleWorkerClick(checkIn) }, checkIn.id))) })] })), groupedWorkers.missed.length > 0 && (_jsxs("div", { children: [_jsxs("h4", { className: "text-sm font-medium text-red-600 mb-3", children: ["MISSED CHECK-INS (", groupedWorkers.missed.length, ")"] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4", children: groupedWorkers.missed.map((checkIn) => (_jsx(WorkerStatusCard, { checkIn: checkIn, onClick: () => handleWorkerClick(checkIn) }, checkIn.id))) })] })), groupedWorkers.overdue.length > 0 && (_jsxs("div", { children: [_jsxs("h4", { className: "text-sm font-medium text-yellow-600 mb-3", children: ["OVERDUE (", groupedWorkers.overdue.length, ")"] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4", children: groupedWorkers.overdue.map((checkIn) => (_jsx(WorkerStatusCard, { checkIn: checkIn, onClick: () => handleWorkerClick(checkIn) }, checkIn.id))) })] })), groupedWorkers.safe.length > 0 && (_jsxs("div", { children: [_jsxs("h4", { className: "text-sm font-medium text-green-600 mb-3", children: ["SAFE (", groupedWorkers.safe.length, ")"] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4", children: groupedWorkers.safe.map((checkIn) => (_jsx(WorkerStatusCard, { checkIn: checkIn, onClick: () => handleWorkerClick(checkIn) }, checkIn.id))) })] }))] }));
}
